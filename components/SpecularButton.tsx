...
import { acquireSpecularContext, onSpecularSlot, releaseSpecularContext } from "@/lib/specular-slots";
import "./SpecularButton.css";

type SpecularSize = "sm" | "md" | "lg";
type SpecularType = "button" | "submit" | "reset";
...
export function SpecularButton({
  ...
  const propsRef = useRef<SpecularConfig>({
    radius,
    lineColor,
    baseColor,
    intensity,
  ...
  useEffect(() => {
    ...
    const startGl = () => {
      if (stopGl) return;
      if (!acquireSpecularContext()) {
        cancelWait ??= onSpecularSlot(startGl);
        return;
      }
      cancelWait?.();
  ...
  const cls = `btn specular-button ${sizeClass(size)}${className ? ` ${className}` : ""}`;
  const inner = (
    <>
